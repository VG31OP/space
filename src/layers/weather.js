...
const WEATHER_URL = (lat, lng) => `/api/weather?lat=${lat}&lon=${lng}`;
    ...
    try {
        const results = await Promise.allSettled(
            WEATHER_GRID.map(async (loc) => {
                const data = await robustFetch(WEATHER_URL(loc.lat, loc.lng));
        ...
        results.forEach(r => {
            ...
            const windDir = weather.windspeed > 5 ? `\u{1F4A8} ${Math.round(weather.windspeed)}m/s` : '';
            const temp = `\u{1F321} ${weather.temperature}\u{00B0}C`;
            const wmo = weather.weathercode;
            const icon = wmo === 0 ? '\u{2600}' : wmo < 50 ? '\u{26C5}' : wmo < 70 ? '\u{1F327}' : wmo < 80 ? '\u{2744}' : '\u{26C8}';
        ...
        console.warn('Weather layer failed:', e);
    }
